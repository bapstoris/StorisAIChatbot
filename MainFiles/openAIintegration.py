...
# Create a fake response object mimicking the real one (remove later)
class FakeResponse:
    def __init__(self):
        self.choices = [MagicMock()]
        self.choices[0].message = MagicMock()
        self.choices[0].message.content = (
            "Keywords: AI, program, Christopher Strachey, checkers, Ferranti Mark I, machine learning, Shopper, rote learning, Arthur Samuel"
        )

# Initialize the client
client = OpenAI()

# Mock the 'create' method (remove later)
client.chat.completions.create = MagicMock(return_value=FakeResponse())

# Your code calls the mocked method instead of the real API
